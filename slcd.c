...
void slcdInitialize(void){
	...
	PORTB->PCR[23] = PORT_PCR_MUX(0UL);      //zgodnie z instrukcja
...
/*----------------------------------------------------------------------------
  Function for printing dp
 *----------------------------------------------------------------------------*/
void slcd_print_dp(uint8_t position){  //wlacz kropke na wybranej pozycji
   switch(position){
    case 1:
      LCD->WF8B[17] |= 0x01;
      break;
...
void slcdDisplay(uint16_t value ,uint16_t format){	
	...
	while (value != 0) {                  //algorytm zamiany na dowolny system liczbowy (reszta z dzielenia i dzielenie calkowite)
		tab[i] = value % format;      //reszte wpisz jako kolejna liczbe wyniku
		value = value / format;       //wynik calkowity do kolejnych dzielen
		i--;
	}
	
	for(i=0; i < 4; i++){		
		slcdSet(tab[i], i+1);         //wypis na lcd
	}
}
/*----------------------------------------------------------------------------
  Clear sLCD
